...
def monte_carlo_validation(X, y, hidden_neurons=10, R=250):
    ...
    return {
        'media': np.mean(mse_list),
        'desvio': np.std(mse_list),
        'maior': np.max(mse_list),
        'menor': np.min(mse_list)
    }

resultado_mlp = monte_carlo_validation(X_norm, y_norm, hidden_neurons=10)
print("Resultado da Validação Monte Carlo (MLP):")
print(resultado_mlp)
