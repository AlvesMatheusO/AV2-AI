...
def monte_carlo_rbf(X, Y_labels):
    R = 100
    accs = []
    sens_list = []
    spec_list = []
    ...
    for _ in range(R):
        ...
        model = RBF(input_dim=X_train.shape[0], hidden_dim=10, output_dim=3, sigma=1.0)
...
# execução
accs, sens, specs = monte_carlo_rbf(X, labels)
