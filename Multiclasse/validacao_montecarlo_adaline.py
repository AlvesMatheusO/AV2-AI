...
def monte_carlo_adaline(X, Y_labels):
    R = 100
    accs = []
    sens_list = []
    spec_list = []
    ...
    for _ in range(R):
        ...
        for i in range(Y_train_bipolar.shape[0]):
            model = Adaline(input_dim=X_train.shape[0], learning_rate=0.01, max_epochs=1000)
            model.train(X_train, Y_train_bipolar[i])
            models.append(model)

        outputs = [model.w @ X_test for model in models]
...
# execução
accs, sens, specs = monte_carlo_adaline(X, labels)
